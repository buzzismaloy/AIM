...
2. На стадии **практического использования** выполняются следующие действия:
    ...
    3. Выходам нейронов второго слоя в качестве начальных величин присваиваются значения выходов нейронов первого слоя, полученные на предыдущем шаге: ![gallery](pics/metoda24.png)

        Далее первый слой нейронов на стадии практического использования больше не задействуется

    4. Для каждой итерации q рассчитываются новые значения состояний и выходов нейронов второго слоя. Состояния нейронов определяются по соотношению:

        ![gallery](pics/metoda25.png)

        или в матричной форме записи:

        ![gallery](pics/metoda26.png)

        ![gallery](pics/metoda27.png)

    5. Цикл в п. 2.4 повторяется до стабилизации выходного вектора в соответствии с условием:

        ![gallery](pics/metoda28.png)

---

В идеальном случае после стабилизации должен получиться выходой вектор с одним положительным и всеми остальными нулевыми элементами. Индекс единственного положительного элемента непосредственно указывает на класс неизвестного входного образа.
